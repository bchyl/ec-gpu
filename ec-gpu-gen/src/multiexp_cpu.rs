...
use group::Group as GroupEngine;
...
pub trait QueryDensity: Sized {
    ...
    fn generate_exps<GE: GroupEngine>(
        self,
        exponents: Arc<Vec<<<GE as GroupEngine>::Scalar as PrimeField>::Repr>>,
    ) -> Arc<Vec<<<GE as GroupEngine>::Scalar as PrimeField>::Repr>>;
}
...
mod tests {
    ...
    fn test_with_bn254() {
        ...
        let v: Vec<<Bn256 as GroupEngine>::Scalar> = (0..SAMPLES)
            .map(|_| <Bn256 as GroupEngine>::Scalar::random(&mut *rng))
            .collect();
        let g = Arc::new(
            (0..SAMPLES)
                .map(|_| <Bn256 as GroupEngine>::G1::random(&mut *rng).to_affine())
                .collect::<Vec<_>>(),
        );
