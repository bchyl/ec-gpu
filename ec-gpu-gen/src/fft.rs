...
impl<'a, G: Group + GpuEngine> SingleFftKernel<'a, G> {
    ...
    pub fn create(
        ...
        let source = match device.vendor() {
            Vendor::Nvidia => crate::gen_source::<G, Limb32>(),
            _ => crate::gen_source::<G, Limb64>(),
        };
        let program = program::program::<G>(device, &source)?;
    ...
    pub fn radix_fft(
        ...
        let closures = program_closures!(|program, input: &mut [G::Scalar]| -> EcResult<()> {
            let n = 1 << log_n;
